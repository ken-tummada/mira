...
const ABS = (path?: string) =>
  path?.startsWith("http") ? path : path ? new URL(path, NEWS_URL).toString() : undefined;
...
function extractSectionHeadlines(doc: Document): SectionHeadline[] {
  ...
  const h2s = Array.from(day.querySelectorAll(":scope > h2")) as HTMLHeadingElement[];
  ...
  for (const h2 of h2s) {
    ...
    clone.querySelectorAll(".sources, .related, .action-bar, script, style").forEach((el) => el.remove());
    const text = clean(clone.textContent || "");
    if (!text || text.length < 40) continue;

    const link = ABS(article.querySelector(".action-bar .direct-link a")?.getAttribute("href") || undefined);
  ...
  if (picked.length === 0) {
    const articles = Array.from(day.querySelectorAll(":scope > article")).slice(0, 3);
    for (const art of articles) {
      ...
      const text = clean(clone.textContent || "");
      if (!text || text.length < 40) continue;
      const link = ABS(art.querySelector(".action-bar .direct-link a")?.getAttribute("href") || undefined);
...
async function fetchSectionHeadlines(): Promise<SectionHeadline[]> {
  const res = await fetch(PROXY + encodeURIComponent(NEWS_URL), { cache: "no-store" });
